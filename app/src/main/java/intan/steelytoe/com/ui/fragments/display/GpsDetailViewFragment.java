...
import intan.steelytoe.com.common.events.RecognitionReciverService;
import intan.steelytoe.com.common.events.RecognitionService;
import intan.steelytoe.com.common.events.ServiceBackground;
import intan.steelytoe.com.ui.activity.ViewTrackActivity;
...
public class GpsDetailViewFragment extends Fragment{
    ...
    public void onStart() {
        super.onStart();
        getActivity().setTitle("Steelytoe");
    ...
    public void loadService(){
        ...
        if(checkGps() == 0 ) {
            ...
            if (checkSession() == 1) {
                ...
                getActivity().stopService(service); // stop location service
                getActivity().stopService(recSercvice); // stop activity recognition service
                ...
                // Proses Start
                prefManager.setRunningId(null); // if null, then the running id will be current datetime

                editor.putString("session", "1");
                editor.commit();
                setActionButtonStop__();

                getActivity().startService(service); // start location service
                getActivity().startService(recSercvice); // start activity recognition service
    ...
    private void saveRunningDetail() {
        CrudRunningDetail crudRunningDetail = CrudRunningDetail.getInstance(getContext());
        crudRunningDetail.setRunningDetail(prefManager.getRunningId(),
                prefManager.getStartedTime(),String.valueOf(prefManager.getLastDistance()),
                textTimer.getText().toString(), prefManager.getDateHeader());
        prefManager.removeRunningId();
    }
